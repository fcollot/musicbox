...
from PySide@MBOX_PYSIDE_VERSION_MAJOR@.QtWidgets import QLabel, QSlider, QVBoxLayout, QWidget
...
class Viewer(QWidget):
    ...
    def __init__(self, image=None):
        ...
        if image:
            ...
            self.image = image
            self.sliceMapper.SetInputData(image.vtk_image)
            self.sliceMapper.SetOrientationToZ()
            self.sliceMapper.SetSliceNumber(0)
            self.sliceMapper.Update()

            self.sliceSlider.setMaximum(image.vtk_image.GetDimensions()[2] - 1)
            self.currentSlice = 0
            self.updateSlice()
